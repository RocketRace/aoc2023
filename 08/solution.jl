...
for (initial_state, _) in [(from, to) for (from, to) in rules if endswith(from, "A")]
    ...
    while true
        ...
        if (new_state, i % length(path)) in visited
            offset = first(findall(x -> x == (new_state, i % length(path)), journey))
            push!(offsets, offset - 1)
            push!(loop_lengths, length(journey) - offset)
            z_index = first(findall(x -> endswith(x[1], "Z"), journey)) - 1
            push!(zs, z_index)
            break
        else
...
println(lcm([BigInt(loop_length) for loop_length in zs]))
